...
### 2.4 *Marcus* — the moderator (admin role)
...
### 3.2 Browse the home feed

```
Home (3)
   ├─ see 5 horizontal stories + scrollable post cards
   ├─ tap story → open full-screen story (future v2; v1: tap → contacts / profile preview)
   ├─ tap post card → open Hidden Discussion overlay (screen 9) if locked, or Results Reveal (screen 11) if ended
   └─ tap FAB → Create Post (screen 4)
```
...
Home (3) → tap FAB → Create Post (4)
   ...
   └─ "Where do you want to discuss this?" (v1 = always group; v2 = public too)
   └─ → Select Contacts (5) → multi-pick
   └─ → Group Invitation (6) → send invites
   └─ → if group exists: Group Exists dialog (7) → "Go to Group" / "Cancel"
   └─ → Set Result Timer (8) → 30 m / 1 h / 3 h / custom
   └─ → Publish
   └─ → back to Home with new post visible
```
...
Post is published → timer starts
   └─ only invited group + author can see the post AND its responses
   └─ responders submit via Hidden Discussion sheet (9)
   └─ other users tapping the post card see a "Locked responses · 00:45:12" placeholder
Countdown Timer (10) shows in-app for everyone with access
Timer hits 0
   └─ responses are unhidden → Results Reveal (11) is the new view for everyone
   └─ notifications fan out: "Results are available for your discussion"
```
...
| --- | --- | --- | --- |
| 1 | Splash | `/` (auth) | Logo + tagline + auto-advance |
| 2 | Login | `/(auth)/login` | Country picker + phone + Send OTP |
| – | Verify OTP | `/(auth)/verify-otp` | Not in references but required by flow |
| 3 | Home Feed | `/(app)/home` | Stories + post cards + FAB |
| 4 | Create Post | `/(app)/create` | Modal sheet, Image/Video/Audio/More |
| 5 | Select Contacts | `/(app)/create/contacts` | Search + multi-pick chips |
| 6 | Group Invitation | `/(app)/create/invites` | Timeline of accept/pending/rejected |
| 7 | Group Exists | `/(app)/create/group-exists` | Modal confirmation |
| 8 | Set Result Timer | `/(app)/create/timer` | 30m / 1h / 3h / custom |
| 9 | Hidden Discussion | `/(app)/post/[id]` | Locked responses sheet |
| 10 | Countdown Timer | `/(app)/post/[id]/countdown` | Full-screen timer |
| 11 | Results Reveal | `/(app)/post/[id]/results` | After timer ends |
...
| 16 | Admin Dashboard | `/(admin)/dashboard` | Server-rendered web (out of mobile scope) |
| 17 | Settings | `/(app)/settings` | Toggles, logout |

The Admin Dashboard (16) is **web-only** for v1 — same backend, separate React SPA. Mobile does not ship admin screens.
...
- JWT-based auth (access + refresh), refresh token in `expo-secure-store`
- Story rings (read-only on home; full story posting is v2)
- Post creation with image / video / audio / pdf attachments
- Group creation flow (contacts → invitations → timer)
- Hidden-discussion countdown mechanism
- Results Reveal after timer ends
- Reactions (likes) + comments on revealed posts
- Notifications (in-app; push is v2)
- Reporting with 4 reasons
- Profile screen with stats
- Settings screen (toggles only; dark mode v2)
- Admin dashboard (web) with reported-posts queue

### 5.2 Out-of-scope (explicit non-goals for MVP)

- Reels / short-form video
- DMs (1:1 chat)
- Public, non-group posting
- Hashtags / search ranking algorithm
- Live streaming
- Stories *creation* (only viewing in MVP)
